...
vector<float> AVL<T, S, C>::bias(shared_ptr<node<T, S, C>> root) {
    ...
    is a significant measure because it reveals how many females are less
    likely to be selected by the original shortlisting algorithm. If males
    dominate the left subtree, it implies that females are being biased
    against due to lower experience levels.
    */
    ...
    while (!q.empty()) {
        ...
        float ratio = (males == 0) ? (females > 0 ? females : 0) : (float)females / males;
