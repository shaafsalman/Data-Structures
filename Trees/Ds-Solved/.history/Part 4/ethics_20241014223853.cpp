...
int AVL<T, S, C>::height(shared_ptr<node<T, S, C>> node) {
    if (!node) return 0;
    return node->height; // Return the height of the node directly
}
...
int AVL<T, S, C>::number_to_shortlist(shared_ptr<node<T, S, C>> root) {
    ...
    return height(root); // The number of shortlisted applicants equals the height of the tree
}
...
template <class T, class S, class C>
int AVL<T, S, C>::depth(shared_ptr<node<T, S, C>> node) {
    if (!node) return 0;
