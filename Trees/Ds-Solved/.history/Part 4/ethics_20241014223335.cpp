...
// This function calculates the number of applicants that you can shortlist
template <class T, class S, class C>
int AVL<T, S, C>::number_to_shortlist(shared_ptr<node<T, S, C>> root) {
    if (!root) return 0; // Base case: empty tree
    // Count based on specific criteria, e.g., threshold for work experience
    int count = 0;
    if (root->workExperience > threshold) { // Use your threshold condition here
        count = 1;
    }
    // Add counts from left and right subtrees
    return count + number_to_shortlist(root->left) + number_to_shortlist(root->right);
}
...
vector<float> AVL<T, S, C>::bias(shared_ptr<node<T, S, C>> root) {
    ...
    while (!q.empty()) {
        ...
        // Count males and females in the current node and its children
        ...
        countHelper(current); // Count in the current node and all its children
        ratios.push_back(maleCount > 0 ? static_cast<float>(femaleCount) / maleCount : 0); // Compute ratio
