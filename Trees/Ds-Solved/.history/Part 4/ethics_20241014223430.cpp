...
int AVL<T, S, C>::number_to_shortlist(shared_ptr<node<T, S, C>> root) {
    ...
    // The number of shortlisted applicants equals the number of nodes in the left subtree + 1
    // This assumes that you want to count the node itself and its left subtree for shortlisting.
    int left_count = number_to_shortlist(root->left);
    return left_count + 1; // Count this node as well
}
...
vector<float> AVL<T, S, C>::bias(shared_ptr<node<T, S, C>> root) {
    ...
    while (!q.empty()) {
        ...
        // Count males and females in the current node's subtree
        ...
        countHelper(current); // Count in the current node's subtree

        // Compute and push back the ratio
        ratios.push_back(maleCount > 0 ? static_cast<float>(femaleCount) / maleCount : 0); 
