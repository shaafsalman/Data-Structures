...
vector<T> AVL<T, S, C>::in_order(shared_ptr<node<T, S, C>> root) {
    ...
    // Only return the number of shortlisted candidates based on the limit
    for (size_t i = 0; i < min(limit, temp.size()); ++i) {
...
vector<T> AVL<T, S, C>::level_order(shared_ptr<node<T, S, C>> root) {
    ...
    int limit = number_to_shortlist(root); // Get the limit on number of shortlisted candidates
    return vector<T>(result.begin(), result.begin() + min(limit, result.size())); // Return only up to the limit
}
