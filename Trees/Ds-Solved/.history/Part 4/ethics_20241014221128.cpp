...
int AVL<T, S, C>::number_to_shortlist(shared_ptr<node<T, S, C>> root) {
    ...
    return height(root);  // The number of shortlisted applicants equals the height of the rightmost leaf
}
...
vector<float> AVL<T, S, C>::bias(shared_ptr<node<T, S, C>> root) {
    ...
    while (!q.empty()) {
        ...
        function<void(shared_ptr<node<T, S, C>>)> countHelper = [&](shared_ptr<node<T, S, C>> node) {
            if (node) {
                // Count the gender in the current node
                if (node->gender == "Male") {
                    maleCount++;
                ...
                // Debug print
                cout << "Counting in node: " << node->fullName << " | Male: " << maleCount << " | Female: " << femaleCount << endl;
        ...
        if (maleCount > 0) {
            ...
            ratios.push_back(0.0); // If no males, ratio is defined as 0
        }

        // Debug print for ratio
        cout << "Node: " << current->fullName << " | FemaleCount: " << femaleCount << " | MaleCount: " << maleCount << " | Ratio: " << ratios.back() << endl;
    ...
    while (!q.empty()) {
        ...
        ratios.push_back(femaleCount > 0 ? static_cast<float>(femaleCount) / maleCount : 0); // Compute ratio
