...
void DNS::getDomainPagesHelper(shared_ptr<node<string, string>> currDomain, string currentPath, vector<shared_ptr<Webpage>>& results)
{
    if (currDomain->children.empty()) {
        // If this is a leaf node, add the current path to the results as a Webpage object
        shared_ptr<Webpage> webpage = make_shared<Webpage>(currentPath); // Create Webpage object from the current path
