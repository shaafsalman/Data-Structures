...
void DNS::addWebpage(const string& url)
{
    ...
    {
        // Look for the subdomain node
        auto subdomainNode = domainTree->findKey(subdomain);
        if (!subdomainNode)
        {
            // Create a new node for the subdomain if it doesn't exist
    ...
    // Add the webpage to the current node (the last subdomain or domain node)
    currentNode->addWebpage(webpage); // You may need to implement addWebpage method in node class
}
