...
void DNS::getDomainPagesHelper(shared_ptr<node<string, string>> currDomain, string currentPath, vector<shared_ptr<Webpage>>& results) // Corrected the signature to match dns.h
{
    if (currDomain->children.empty()) {
        ...
        results.push_back(make_shared<Webpage>(domain + "." + tld));
    } else {
