...
struct Webpage
{
    ...
    // Constructor that accepts a single URL
    Webpage(string url)
    {
        int i = 0;
    ...
    Webpage(string domain, vector<string> subdomains, string TLD)
...
public:
    ...
    void getDomainPagesHelper(shared_ptr<node<string, string>> currDomain, string currentPath, vector<shared_ptr<Webpage>> &results);
