...
void DNS::addWebpage(string url)
{
    ...
    auto newWebpage = make_shared<Webpage>(domain, tld);

    shared_ptr<node<string, string>> currNode = domainTree->getRoot();
    while (currNode)
    {
        if (currNode->key == domain)
        {
            currNode->children.push_back(make_shared<node<string, string>>(tld, ""));
            currNode->children.back()->subdomains.push_back(newWebpage);
            return;
        }
    ...
    currNode->children.push_back(make_shared<node<string, string>>(domain, ""));
    currNode->children.back()->subdomains.push_back(newWebpage);
}
