...
void DNS::generateOutput(vector<shared_ptr<Webpage>> pages)
{
    ...
    file << pages.size() << endl; // Print the number of webpages
    for (const auto& webpage : pages)
    {
        string line = webpage->domain + "." + webpage->TLD; // Base URL
        for (const auto& subdomain : webpage->subdomains)
        {
            line += "/" + subdomain; // Append subdomains to the URL
        }
        file << line << endl; // Print the complete URL
    }
...
vector<shared_ptr<Webpage>> DNS::getDomainPages(string domain)
{
    ...
    getDomainPagesHelper(domainNode, domainNode->key, results); // Pass the results vector by reference
    return results;
}
...
void DNS::getDomainPagesHelper(shared_ptr<node<string, string>> currDomain, string currentPath, vector<shared_ptr<Webpage>>& results)
{
    ...
    if (currDomain->children.empty()) {
        // Create a new Webpage object with the correct URL
        shared_ptr<Webpage> webpage = make_shared<Webpage>(currentPath + "." + currDomain->key);
        results.push_back(webpage); // Ensure valid webpage object is added
    }
...
vector<shared_ptr<Webpage>> DNS::getAllWebpages(string TLD)
{
    ...
    for (const auto& domainNode : tldNode->children) {
        getDomainPagesHelper(domainNode, domainNode->key + "." + TLD, results); // Add TLD while constructing path
    }
