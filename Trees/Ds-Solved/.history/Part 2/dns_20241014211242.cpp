...
vector<shared_ptr<Webpage>> DNS::getAllWebpages(const string& TLD)
{
    ...
    if (tldNode == nullptr)
    {
        cout << "TLD not found: " << TLD << endl; // Debug message
...
vector<shared_ptr<Webpage>> DNS::getDomainPages(const string& domain)
{
    ...
    {
        cout << "Domain not found: " << domain << endl; // Debug message
        return results; // Domain not found, return empty vector
    }

    collectDomainPages(domainNode, domainNode->key + "." + domainNode->TLD, results);
    return results;
}
