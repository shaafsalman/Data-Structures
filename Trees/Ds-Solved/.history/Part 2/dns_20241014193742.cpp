...
{
    shared_ptr<Webpage> page = make_shared<Webpage>(url); // Create Webpage object from URL
    string tld = page->TLD;
...
vector<shared_ptr<Webpage>> DNS::getAllWebpages(string TLD)
{
    ...
    for (const auto& domainNode : domains) {
        ...
        string tldCheck = "." + TLD;
        if (domainNode->key.length() > tldCheck.length() && 
            domainNode->key.substr(domainNode->key.length() - tldCheck.length()) == tldCheck) {
