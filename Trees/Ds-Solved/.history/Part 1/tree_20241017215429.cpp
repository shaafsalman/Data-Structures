...
int Tree<T, S>::findHeightHelper(shared_ptr<node<T, S>> currNode) 
{
    ...
    for (int i = 0; i < currNode->children.size(); i++) 
    {
        height = max(height, findHeightHelper(currNode->children[i]) + 1); 
    }
...
shared_ptr<node<T, S>> Tree<T, S>::deleteLeafHelper(shared_ptr<node<T, S>> currNode, T key) 
{
    ...
    for (typename vector<shared_ptr<node<T, S>>>::iterator it = currNode->children.begin(); it != currNode->children.end(); ++it) 
    {
        if ((*it)->key == key) 
        {
    ...
    for (int i = 0; i < currNode->children.size(); i++) 
    {
        if (deleteLeafHelper(currNode->children[i], key) != nullptr) 
        {
            return currNode; 
        }
    }
