...
void StudentDatabase::read_csv(const string &filename) {
    ...
    while (getline(file, line)) {
        ...
        // Ensure valid roll number format (it should only contain digits)
        if (!all_of(rollNumber.begin(), rollNumber.end(), ::isdigit)) {
            cout << "Skipping invalid roll number: " << rollNumber << endl;
            continue;
        }
