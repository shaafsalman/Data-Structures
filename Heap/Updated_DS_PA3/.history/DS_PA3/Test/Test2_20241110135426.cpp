...
int test_extractMin(shared_ptr<MinHeap> heap, vector<int> ref_vec)
{
    ...
    int counter = 0;
    sort(ref_vec.begin(), ref_vec.end());  // Sort the reference vector to match expected extraction order

    int len = ref_vec.size();

    // Extract the minimum from the heap and compare
    for (int i = 0; i < len; i++)
    {
        int min_val = heap->extractMin();
        cout << "Extracted: " << min_val << endl;  // Print the extracted value for debugging

        if (min_val == ref_vec[i])
        {
            counter++;
        }
    }
...
int test_delete(shared_ptr<MinHeap> heap, vector<int> ref_vec)
{
    ...
    // Perform delete operations and check the heap state after each deletion
    int counter = 0;
    int len = ref_vec.size();

    for (int i = 0; i < len; i++)
    {
        heap->extractMin();  // Remove the smallest element for comparison purposes
    }

    // Now check if the heap matches the expected order
    shared_ptr<int> head = heap->getHeap();  // Get the current heap state
    for (int i = 0; i < len; i++)
    {
        if (head.get()[i] == ref_vec[i])
        {
