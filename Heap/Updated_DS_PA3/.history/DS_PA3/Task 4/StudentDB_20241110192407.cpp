...
// Function to read student data from a CSV filevoid StudentDatabase::read_csv(const string &filename) {
    // Open the file and read each line, parsing the details into a Student object.
    // Add each student to the database using the addStudent function.
    ...
    while (getline(file, line)) {
        ...
        addStudent(student);
        insertionCount++;  // Increament only if insertion is successful (you can add a condition here if needed)
    }
    ...
    // Optionally, count the number of students in the database after insertion
