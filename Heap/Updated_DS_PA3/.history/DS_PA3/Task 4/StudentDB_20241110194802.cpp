...
StudentDatabase::StudentDatabase() : LUMS(make_shared<node<int, vector<Tree<int, Student>>>>(0, vector<Tree<int, Student>>())) {
    ...
    for (int batch = 20; batch <= 24; ++batch) {  // Example range for batches
        vector<Tree<int, Student>> houses(4);  // Four houses per batch (HSS, SAHSOL, SSE, SDSB)
...
void StudentDatabase::addStudent(const Student &student) {
    ...
    if (!batchNode) {
        // If the batch doesn't exist, create a new batch with empty school trees
    ...
    // Check if the student already exists in the tree based on batch and school
    if (schoolTree.findKey(schoolRollNumber) != nullptr) {
        cout << "Duplicate entry for student: " << student.name << " with roll number: " << schoolRollNumber << " in batch " << batch << " and school " << schoolCode << endl;
        return;
    }

    // Create a node for the student and insert it into the tree
