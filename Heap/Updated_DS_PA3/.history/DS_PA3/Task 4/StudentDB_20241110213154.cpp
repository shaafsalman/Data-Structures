...
void StudentDatabase::read_csv(const string &filename) {
    ...
    string line;
    int rowCount = 0; // To count the number of rows read
    int studentCount = 0; // To count the number of students in the tree
    ...
    while (getline(file, line)) {
        ...
        rowCount++; // Increment the row count for each student added
    }

    file.close();

    // After reading all the students, calculate the number of students in the tree
    // Iterate over the LUMS structure to count the students
    studentCount = 0;
    ...
    cout << "Total rows read from CSV: " << rowCount << endl;
    cout << "Total students in the tree: " << studentCount << endl;
}
