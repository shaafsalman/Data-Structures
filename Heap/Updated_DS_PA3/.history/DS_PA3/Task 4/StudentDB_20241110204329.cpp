...
void StudentDatabase::addStudent(const Student &student) {
    ...
    try {
        // Locate the batch node
        shared_ptr<node<int, vector<Tree<int, Student>>>> batchNode = LUMS.findKey(batch);
        if (!batchNode) return;  // Batch node must exist as we preinitialize all batches

        int schoolIndex = getSchoolIndex(schoolCode);
        if (schoolIndex == -1) return;  // Invalid school code

        Tree<int, Student> &schoolTree = batchNode->value[schoolIndex];

        // Refined check: Ensure duplicates are only detected within the same batch and roll number
        ...
        if (existingNode && existingNode->value.batch == batch) {
            // Skip insertion if a student with the same batch and school roll number already exists
            return;
        }

        // Create and insert new student node
