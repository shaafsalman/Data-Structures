...
StudentDatabase::StudentDatabase()
    ...
    // Initialize batch nodes with school trees for all batches
    for (int batch = 0; batch < 4; ++batch) {
        ...
        LUMS.insertChild(batchNode, 0); // Ensure the batchNode is inserted into the LUMS root properly
    }
...
void StudentDatabase::addStudent(const Student &student) {
    ...
    // Find the appropriate school tree based on school code
    ...
    // Insert the student node into the correct school tree
    ...
    int schoolIndex = getSchoolIndex(schoolCode);
    Tree<int, Student> &schoolTree = batchNode->value[schoolIndex];
    if (schoolTree.findKey(schoolRollNumber) != nullptr) return;
...
bool StudentDatabase::searchStudent(const string &rollNumber) {
    try {
        ...
        if (!all_of(batchStr.begin(), batchStr.end(), ::isdigit) || !all_of(schoolRollNumberStr.begin(), schoolRollNumberStr.end(), ::isdigit)) throw invalid_argument("Roll number parts must contain only digits.");
