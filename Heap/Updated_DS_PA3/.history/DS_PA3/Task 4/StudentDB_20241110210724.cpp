...
StudentDatabase::StudentDatabase()
    ...
    // Initialize the batch nodes with empty school trees
    for (int batch = 0; batch < 4; ++batch) {
        vector<Tree<int, Student>> schoolTrees(0); // Empty trees for each batch initially

        // Create the batch node with empty school trees
        shared_ptr<node<int, vector<Tree<int, Student>>>> batchNode = 
            make_shared<node<int, vector<Tree<int, Student>>>>(batch, schoolTrees);

        // Insert the batch node into the LUMS root node
        LUMS.insertChild(batchNode, 0); 
    }

    // Read student data from CSV and insert into the database
...
void StudentDatabase::addStudent(const Student &student) {
    ...
    // Find or create the batch node in LUMS
    shared_ptr<node<int, vector<Tree<int, Student>>>> batchNode = LUMS.findKey(batch);
    if (!batchNode) {
        // Batch node not found, create new batch with an empty vector for school trees
        vector<Tree<int, Student>> schoolTrees(0);
        batchNode = make_shared<node<int, vector<Tree<int, Student>>>>(batch, schoolTrees);
        LUMS.insertChild(batchNode, 0);
        cout << "Batch " << batch << " created.\n";
    }

    // Dynamically add a school tree for the given schoolCode
    auto& schoolTrees = batchNode->value;
    if (schoolTrees.empty() || !schoolIndexMap.count(schoolCode)) {
        // Adding a new school tree for this school code
        Tree<int, Student> newSchoolTree(make_shared<node<int, Student>>(0, Student()));
        schoolTrees.push_back(newSchoolTree);
        schoolIndexMap[schoolCode] = schoolTrees.size() - 1;  // Store the index for future use
        cout << "New school added with code " << schoolCode << endl;
    }

    // Get the index of the school tree dynamically based on the school code
    int schoolIndex = schoolIndexMap[schoolCode];
    Tree<int, Student> &schoolTree = schoolTrees[schoolIndex];

    // If student already exists, no need to add
    if (schoolTree.findKey(student.schoolRollNumber) != nullptr) {
        cout << "Student already exists with roll number " << student.schoolRollNumber << endl;
        return;
    }

    // Create a new node for the student and insert into the correct school tree
    shared_ptr<node<int, Student>> studentNode = make_shared<node<int, Student>>(student.schoolRollNumber, student);
    schoolTree.insertChild(studentNode, 0);

    cout << "Student added: " << student.name << " with roll number " << student.schoolRollNumber << endl;
}
...
bool StudentDatabase::searchStudent(const string &rollNumber) {
    try {
        ...
        // Get the school index dynamically
        if (schoolIndexMap.find(schoolCode) == schoolIndexMap.end()) {
            return false;  // School code not found
        }
        int schoolIndex = schoolIndexMap[schoolCode];
...
void StudentDatabase::displayBatchAndSchool(const int batch, const string schoolCode) {
    ...
    // Get the school index dynamically
    if (schoolIndexMap.find(schoolCode) == schoolIndexMap.end()) {
        cout << "School code not found for batch " << batch << ".\n";
        return;
    }
    int schoolIndex = schoolIndexMap[schoolCode];
    Tree<int, Student> &schoolTree = batchNode->value[schoolIndex];
