...
// Helper function to get school index based on school code
...
StudentDatabase::StudentDatabase()
    ...
    // Initialize the batch nodes with school trees for all 4 batches
    for (int batch = 0; batch < 4; ++batch) {
        vector<Tree<int, Student>> schoolTrees = {
            Tree<int, Student>(make_shared<node<int, Student>>(0, Student())), // School 01
            Tree<int, Student>(make_shared<node<int, Student>>(0, Student())), // School 02
            Tree<int, Student>(make_shared<node<int, Student>>(0, Student())), // School 03
            Tree<int, Student>(make_shared<node<int, Student>>(0, Student()))  // School 04
        };

        // Create the batch node with pre-initialized school trees
...
void StudentDatabase::addStudent(const Student &student) {
    ...
    // Find the pre-initialized batch node in LUMS
    ...
    // Find the appropriate school tree based on the school code
    ...
    if (schoolTree.findKey(schoolRollNumber) != nullptr) {
        cout << "Student already exists with roll number " << schoolRollNumber << endl;
        return;
    }
    ...
    cout << "Student added: " << student.name << " with roll number " << schoolRollNumber << endl;
}
...
void StudentDatabase::read_csv(const string &filename) {
    ...
    for (size_t i = 0; i < batches.size(); i++) {
        shared_ptr<node<int, vector<Tree<int, Student>>>> batchNode = batches[i];
        for (size_t j = 0; j < batchNode->value.size(); j++) {
            totalStudentCount += batchNode->value[j].getAllChildren(0).size();
        }
    }
