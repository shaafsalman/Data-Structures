...
void StudentDatabase::addStudent(const Student &student) {
    ...
    cout << "Adding student: " << student.name << ", Roll Number: " << rollNumber << ", School Code: " << schoolCode << endl;

    // Skip batch-based search and go directly to school code search
    shared_ptr<node<int, vector<Tree<int, Student>>>> batchNode = LUMS.findKey(0);  // We don't care about batch, use 0 here
    if (!batchNode) {
        cout << "Batch node not found for the given school code." << endl;
        return;
    }
    ...
    if (schoolIndex == -1) {
        cout << "Invalid school code: " << schoolCode << endl;
        return;
    }
    ...
    // Create a new student node and insert it into the correct school tree
