...
int MinHeap::extractMin()
{
    ...
    std::cout << "Heap after replacing root: ";
    ...
    while (i < heap_size)
    {
        ...
        // If the smallest value is still the current node, the heap is valid
        if (smallest == i)
            break;

        // Swap the current node with the smallest child
        std::swap(harr.get()[i], harr.get()[smallest]);
        i = smallest; // Move down to the child that was swapped

        // Output heap after reheapification
        std::cout << "Heap after reheapifying: ";
...
void MinHeap::decreaseKey(int i, int new_val)
{
    ...
    {
        std::swap(harr.get()[i], harr.get()[parent(i)]);  // Swap with parent
        i = parent(i);
    }
}
