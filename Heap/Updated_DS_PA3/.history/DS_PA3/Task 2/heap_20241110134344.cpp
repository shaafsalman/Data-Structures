...
{
    // If the new value is greater than the current value, return
    if (new_val > harr.get()[i])
        return;

    // Set the value at index i to the new value
    harr.get()[i] = new_val;
...
{
    // Decrease the value to INT_MIN (or another very small value)
