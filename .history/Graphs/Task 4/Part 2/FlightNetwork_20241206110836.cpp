...
void Airport::addDepartureFlight(string flightNumber, shared_ptr<Airport> destination, int cost, int distance, FlightStatus status) {
    ...
    // Add airports to the departure distance and cost graphs if not already present
    try {
        // Add source and destination airports to the graph
        ...
        // Add the edge between source and destination airports for both distance and cost
...
void Airport::addArrivalFlight(string flightNumber, shared_ptr<Airport> source, int cost, int distance, FlightStatus status) {
    ...
    // Add airports to the arrival distance and cost graphs if not already present
    try {
        // Add source and destination airports to the graph
...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    // Add to departure graphs (distance and cost)
    try {
        // Add airports to the graph if not already present
        ...
        departureFlights_cost->addVertex(flight->getDepartureAirport());
        departureFlights_cost->addVertex(flight->getDestinationAirport());
...
void Airport::addArrivalFlight(shared_ptr<Flight> flight) {
    ...
    // Add to arrival graphs (distance and cost)
    try {
        // Add airports to the graph if not already present
        ...
        arrivalFlights_cost->addVertex(flight->getDepartureAirport());
        arrivalFlights_cost->addVertex(flight->getDestinationAirport());
...
vector<shared_ptr<Airport>> FlightNetwork::getShortestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    // Get the shortest path from the graph
    ...
    if (path.empty()) {
        std::cerr << "No path found between airports!" << std::endl;
        return result;  // Return empty if no path is found
    }

    // Convert vertices to airports and add to the result vector
...
vector<shared_ptr<Airport>> FlightNetwork::alternateRouteForFlight(shared_ptr<Flight> flight) {
    ...
    // Get the new distance and cost for the alternate route
    ...
    // Return the shortest path if it's a better route (less increase in distance and cost), otherwise return the cheapest
    if (percentageIncrease < 0) {
        return shortest; // If the alternate route (cheapest) has a higher increase, return the shortest path
    } else {
        return cheapest; // Otherwise, return the cheapest path
    }
}
