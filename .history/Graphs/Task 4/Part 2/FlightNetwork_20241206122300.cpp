...
void FlightNetwork::addFlight(shared_ptr<Flight> flight) {
    ...
    // Add the flight in both directions for distance and cost graphs
    AirportNetwork_distance.addEdge(departure, destination, flight->getDistance());
    AirportNetwork_cost.addEdge(departure, destination, flight->getCost());
    
    // Add the reverse direction if the flight is bidirectional
    AirportNetwork_distance.addEdge(destination, departure, flight->getDistance());
    AirportNetwork_cost.addEdge(destination, departure, flight->getCost());
}
...
for (const auto& airport : Airports) {
    ...
    // Get the shortest path from the AirportNetwork_distance
    ...
    if (path.empty()) {
        std::cerr << "No path found between " << source->getName() << " and " << destination->getName() << std::endl;
