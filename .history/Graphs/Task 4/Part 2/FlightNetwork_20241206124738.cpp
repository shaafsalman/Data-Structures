...
vector<shared_ptr<Airport>> FlightNetwork::getShortestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    // Debugging print statements (kept as in the original code)
    ...
    std::unordered_map<std::shared_ptr<Airport>, int> distances;
    ...
    std::shared_ptr<Airport> shortestPathDestination = nullptr;
    ...
    while (!queue.empty()) {
        ...
        if (currentAirport == destination) {
            if (distances[currentAirport] < shortestPathLength) {
                shortestPathLength = distances[currentAirport];
                shortestPathDestination = currentAirport;
            }
        }

        // Process all neighboring airports (through all available flights)
        for (const auto& flight : currentAirport->getAllFlights()) {
            ...
            int newDistance = distances[currentAirport] + 1;

            // If we haven't visited or found a shorter path to this airport
            if (visited.find(neighborAirport) == visited.end() || 
                newDistance < distances[neighborAirport]) {
    ...
    if (shortestPathDestination) {
        vector<shared_ptr<Airport>> path;
        auto currentAirport = shortestPathDestination;
