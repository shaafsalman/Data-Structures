...
void FlightNetwork::addFlight(shared_ptr<Flight> flight) {
    ...
    // Ensure both airports are added to the network (if they are not already)
    addAirport(departureAirport);
    addAirport(destinationAirport);

    // Add the flight to the departure and arrival lists of each airport
    departureAirport->addDepartureFlight(flight);
    destinationAirport->addArrivalFlight(flight);

    // Add the edges for both directions (departure -> destination and destination -> departure)
    ...
    // Add the reverse direction for bidirectional travel
