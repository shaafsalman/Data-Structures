...
vector<shared_ptr<Airport>> FlightNetwork::getShortestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    // Debugging: Print unique flights for each airport
    std::cout << "-Network----------------------------------------------------" << std::endl;
    std::unordered_set<std::string> printedFlights;
    for (const auto& airport : Airports) {
        std::cout << "Airport: " << airport->getName() << std::endl;
        for (const auto& flight : airport->getAllFlights()) {
            std::string flightKey = flight->getFlightNumber() + 
                                    flight->getDepartureAirport()->getName() + 
                                    flight->getDestinationAirport()->getName();
            
            // Only print unique flights
            if (printedFlights.find(flightKey) == printedFlights.end()) {
                ...
                printedFlights.insert(flightKey);
            }
        }
    }
    std::cout << "-----------------------------------------------------" << std::endl;
    ...
    std::queue<std::vector<std::shared_ptr<Airport>>> queue;
    std::unordered_set<std::shared_ptr<Airport>> visited;

    // Start with the source airport in the initial path
    queue.push({source});
    ...
    while (!queue.empty()) {
        ...
        auto currentAirport = currentPath.back();

        // If we reached the destination, return the path
        if (currentAirport == destination) {
            // Print the path
            std::cout << "Shortest Path: ";
            for (const auto& airport : currentPath) {
                std::cout << airport->getName() << " ";
            }
            std::cout << std::endl;
            std::cout << "Path Length: " << (currentPath.size() - 1) << std::endl;

            return currentPath;
        }

        // Explore all unique flights from the current airport
        std::unordered_set<std::shared_ptr<Airport>> uniqueDestinations;
        for (const auto& flight : currentAirport->getAllFlights()) {
            auto neighborAirport = flight->getDestinationAirport();

            // Ensure we don't revisit airports and don't add the same destination multiple times
            if (visited.find(neighborAirport) == visited.end() && 
                uniqueDestinations.find(neighborAirport) == uniqueDestinations.end()) {
                ...
                uniqueDestinations.insert(neighborAirport);
            }
        }
    }
