...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    if (!flight->getDepartureAirport() || !flight->getDestinationAirport()) {
    std::cerr << "Invalid flight details provided." << std::endl;
    return;
    }
...
void FlightNetwork::addAirport(shared_ptr<Airport> airport) {
    //  std::cout << "Airports before adding: ";
    for (const auto& existingAirport : Airports) {
        // std::cout << existingAirport->getName() << " ";
    }
    std::cout << std::endl;
...
vector<shared_ptr<Airport>> FlightNetwork::getShortestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    ...
    std::cout << "Shortest path between " << source->getName() 
...
vector<shared_ptr<Airport>> FlightNetwork::getCheapestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    ...
    std::cout << "Cheapest path between " << source->getName() 
              << " and " << destination->getName() << ": ";
    for (const auto& airport : path) {
    }
    std::cout << std::endl;
...
shared_ptr<Airport> FlightNetwork::getBusiestAirport() {
    ...
    for (const auto& airport : Airports) {
        ...
        // Print flights with weights and costs for each airport
        std::cout << "Airport: " << airport->getName() << std::endl;
        for (const auto& flight : airport->getAllFlights()) {
            std::cout << "Flight: " << flight->getFlightNumber()
                      << " - Distance: " << flight->getDistance()
                      << " - Cost: " << flight->getCost() << std::endl;
        }

        // Find busiest airport based on total flights
    ...
    // Handle case where no busiest airport is found
    if (busiest == nullptr) {
        std::cout << "No busiest airport found (no flights available)." << std::endl;
    }
