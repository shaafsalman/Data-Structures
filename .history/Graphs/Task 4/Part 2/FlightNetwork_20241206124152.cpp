...
vector<shared_ptr<Airport>> FlightNetwork::getShortestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    ...
    // Using BFS to find the shortest path (in terms of hops)
      std::unordered_map<shared_ptr<Airport>, shared_ptr<Airport>> cameFrom;
    std::unordered_map<shared_ptr<Airport>, double> gScore;
    std::unordered_map<shared_ptr<Airport>, double> fScore;

    auto compare = [](const std::pair<shared_ptr<Airport>, double>& left, const std::pair<shared_ptr<Airport>, double>& right) {
        return left.second > right.second;
    };
    std::priority_queue<std::pair<shared_ptr<Airport>, double>, std::vector<std::pair<shared_ptr<Airport>, double>>, decltype(compare)> openSet(compare);

    openSet.push({source, 0.0});
    gScore[source] = 0.0;
    fScore[source] = heuristic(source, destination);  // assuming you have a heuristic function
    ...
    while (!openSet.empty()) {
        ...
        for (const auto& flight : current->getAllFlights()) {
            auto neighbor = flight->getDestinationAirport();
            double tentativeGScore = gScore[current] + flight->getDistance();

            if (gScore.find(neighbor) == gScore.end() || tentativeGScore < gScore[neighbor]) {
                cameFrom[neighbor] = current;
                gScore[neighbor] = tentativeGScore;
                fScore[neighbor] = gScore[neighbor] + heuristic(neighbor, destination);  // again assuming heuristic
