...
vector<shared_ptr<Airport>> FlightNetwork::getFlightPlan(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    // Get the shortest path and cheapest path based on different criteria
    auto shortest = getShortestPath(source, destination);
    auto cheapest = getCheapestPath(source, destination);

    // If both paths are the same, return that path
    if (shortest == cheapest) {
        std::cout << "Both paths are identical." << std::endl;
        return shortest;
    }

    // Debugging: Display both paths and calculate their total distance and cost
    int totalShortestDistance = 0;
    int totalCheapestCost = 0;

    std::cout << "Shortest Path (by distance): ";
    for (size_t i = 0; i < shortest.size(); ++i) {
        std::cout << shortest[i]->getName();
        if (i < shortest.size() - 1) {
            std::cout << " -> ";
        }

        // Add the distance for each leg of the shortest path
        if (i < shortest.size() - 1) {
            auto nextAirport = shortest[i + 1];
            for (const auto& flight : shortest[i]->getAllFlights()) {
                if (flight->getDestinationAirport() == nextAirport) {
                    totalShortestDistance += flight->getDistance();
                    break;
                }
            }
        }
    }
    std::cout << " | Total Distance: " << totalShortestDistance << " km" << std::endl;

    std::cout << "Cheapest Path (by cost): ";
    for (size_t i = 0; i < cheapest.size(); ++i) {
        std::cout << cheapest[i]->getName();
        if (i < cheapest.size() - 1) {
            std::cout << " -> ";
        }

        // Add the cost for each leg of the cheapest path
        if (i < cheapest.size() - 1) {
            auto nextAirport = cheapest[i + 1];
            for (const auto& flight : cheapest[i]->getAllFlights()) {
                if (flight->getDestinationAirport() == nextAirport) {
                    totalCheapestCost += flight->getCost();
                    break;
                }
            }
        }
    }
    std::cout << " | Total Cost: " << totalCheapestCost << " USD" << std::endl;

    // Calculate the percentage increase in shortest vs cheapest path
    double percentageIncreaseShortest = (totalShortestDistance / (double)totalCheapestCost - 1) * 100;
    double percentageIncreaseCheapest = (totalCheapestCost / (double)totalShortestDistance - 1) * 100;

    std::cout << "Percentage increase in Shortest Path vs Cheapest Path: " << percentageIncreaseShortest << "%" << std::endl;
    std::cout << "Percentage increase in Cheapest Path vs Shortest Path: " << percentageIncreaseCheapest << "%" << std::endl;

    // Choose the path with the least percentage increase in distance/cost
    if (percentageIncreaseShortest < percentageIncreaseCheapest) {
        std::cout << "Choosing Shortest Path." << std::endl;
        return shortest;
    } else {
        std::cout << "Choosing Cheapest Path." << std::endl;
        return cheapest;
    }
}
