...
void test_domain_pages()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        // Open the output file to compare results
        f1.open("output.txt", ios::in);
        getline(f1, s1);
        int length = stoi(s1);

        cout << "\nDomain: " << domains[i] << endl; // Indicate which domain is being tested
        cout << "Expected Number of Pages: " << num_domains[i] << " | Actual: " << length << endl;
        ...
        if (length == num_domains[i])
        {
            for (int k = 0; k < length; k++)
            {
                ...
                // Output for debugging with proper formatting
                cout << "\n  Output Line " << k + 1 << ": " << s1 << endl;
                cout << "  Expected Line " << k + 1 << ": " << s2 << endl;

                if (s1 != s2)
                {
                    cout << "\033[31m"; // Red text for failure
                    cout << "Mismatch found for domain " << domains[i] << " at line " << k + 1 << endl;
        ...
        {
            cout << "\033[31m"; // Red text for failure
            cout << "Expected number of pages (" << num_domains[i] << ") does not match actual (" << length << ")." << endl;
            cout << "\033[0m"; // Reset color
            return;
        }
        ...
        {
            points++;
            cout << "\033[32m"; // Green text for success
            cout << "Success: Domain " << domains[i] << " matched expected output." << endl;
            cout << "\033[0m"; // Reset color
        }
    ...
    cout << "\n\033[34mTotal Points: " << points << " / 5" << "\033[0m" << endl;
}
