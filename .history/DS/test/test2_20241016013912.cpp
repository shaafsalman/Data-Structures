...
void test_domain_pages() {
    DNS t2; // Assuming DNS is properly defined and implemented
    fstream f1, f2;
    string s1, s2;
    int points = 0, flag;
    part = 2;
    ...
    load_webpages(50, &t2); // Assuming this loads the required data into the DNS object
    ...
    for (int i = 0; i < 5; i++) {
        flag = 1;
        test_case = i + 1;
        t2.generateOutput(t2.getDomainPages(domains[i])); // Generate the output for the current domain
        f1.open("output.txt", ios::in);

        getline(f1, s1);
        int length = stoi(s1);

        // Debug: Show the expected number of domains
        cout << "Testing domain: " << domains[i] << endl;
        cout << "Expected number of webpages: " << num_domains[i] << ", Actual length read: " << length << endl;
        ...
        if (length == num_domains[i]) {
            for (int k = 0; k < length; k++) {
                ...
                // Debug: Show what was read from the output and what was expected
                cout << "Output: " << s1 << " | Expected: " << s2 << endl;

                if (s1 != s2) {
                    flag = 0; // Set flag to indicate a mismatch
                    break;
                }
            }
        } else {
            flag = 0; // Set flag if the length does not match
        }
        ...
        if (flag) {
            ...
            cout << "\033[31mMismatch detected for domain: " << domains[i] << "\033[0m" << endl; // Print error in red
        }
    ...
    cout << "\033[34m"; // Blue color for output
