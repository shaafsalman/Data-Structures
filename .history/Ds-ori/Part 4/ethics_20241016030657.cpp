...
vector<float> AVL<T, S, C>::bias(shared_ptr<node<T, S, C>> root)
{
    ...
    while (!q.empty()) {
        ...
        // Count males and females in the left subtree only if it exists
        if (current->left) {
            auto [males, females] = countGenders(current->left);

            // Calculate the bias ratio: Females / (Males + Females)
            // If there are no males, but females exist, return 1.
            float ratio = (males + females == 0) ? 0 : (males == 0 ? 1 : static_cast<float>(females) / (males + females));
            result.push_back(ratio);
        } else {
            // No left subtree, push a ratio of 0 (or however you want to handle this case)
            result.push_back(0);
        }
