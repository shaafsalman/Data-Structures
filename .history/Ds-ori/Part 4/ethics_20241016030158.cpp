...
vector<float> AVL<T, S, C>::bias(shared_ptr<node<T, S, C>> root)
{
    ...
    // Get the number of applicants that should be processed (based on right-most path height)
    ...
    // Level-order traversal to calculate bias at each node, stopping at the limit
    queue<shared_ptr<node<T, S, C>>> q;
    q.push(root);
    while (!q.empty() && result.size() < limit) {
        auto current = q.front();
        ...
        // Calculate the bias ratio: Females / (Males + Females), avoid division by zero
        float ratio = (males + females == 0) ? 0 : static_cast<float>(females) / males;
