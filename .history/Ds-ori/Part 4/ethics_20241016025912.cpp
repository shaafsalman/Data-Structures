...
vector<float> AVL<T, S, C>::bias(shared_ptr<node<T, S, C>> root)
{
    ...
    auto countGenders = [](shared_ptr<node<T, S, C>> subtree) -> pair<int, int> {
        ...
        while (!q.empty()) {
            auto current = q.front();
            q.pop();
            if (current->gender == "M") males++;  // Use string comparison
            else if (current->gender == "F") females++;  // Use string comparison
            if (current->left) q.push(current->left);
            if (current->right) q.push(current->right);
        }
    ...
    while (!q.empty()) {
        ...
        // Calculate the bias ratio: Females / Males
        float ratio = (males == 0) ? (females > 0 ? static_cast<float>(females) : 0) : static_cast<float>(females) / males;
