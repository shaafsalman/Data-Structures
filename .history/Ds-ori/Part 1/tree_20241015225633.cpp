...
{
    if (root && root->key == key) return false;  // Cannot delete root node

    root = deleteLeafHelper(root, key);  // Attempt to delete the leaf
    return root != nullptr;  // Return true if the root is still valid (indicating success)
}
...
shared_ptr<node<T, S>> Tree<T, S>::deleteLeafHelper(shared_ptr<node<T, S>> currNode, T key)
{
    ...
    // Check for leaf node to delete
    for (auto it = currNode->children.begin(); it != currNode->children.end(); ++it) {
        if ((*it)->key == key) {
            if ((*it)->children.empty()) {  // Only delete if it's a leaf node
                currNode->children.erase(it);  // Erase the leaf node
                return currNode;  // Return the current node after deletion
            } else {
                return currNode;  // Not a leaf, return without deletion
            }
        } else {
