...
// Helper function to recursively collect domain pages with TLD
void DNS::getDomainPagesHelper(shared_ptr<node<string, string>> currDomain, string currentPath, string TLD, shared_ptr<vector<string>> results)
{
    // If this is a leaf node, add the current path with TLD (webpage) to the results
    if (currDomain->children.empty()) {
        results->push_back(currentPath + "." + TLD); // Add the path as a string with TLD
    }

    // Traverse each child (subdomain) and continue building the path
    for (const auto& child : currDomain->children) {
        getDomainPagesHelper(child, currentPath + "/" + child->key, TLD, results);
    }
}
