    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        if (length == num_domains[i])
        {
            for (int k = 0; k < length; k++)
            {
                ...
                getline(f2, s2);
                // Debugging output
                cout << "Expected: " << s2 << endl;  // Show expected result from answers.txt
                cout << "Received: " << s1 << endl;  // Show received result from output.txt

                if (s1 != s2)
                {
                    flag = 0; // Set flag to 0 on mismatch
        ...
        {
            cout << "Expected number of webpages: " << num_domains[i] << endl;
            cout << "Received number of webpages: " << length << endl;
            return; // Exit if length does not match
        }
