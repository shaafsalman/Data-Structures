...
void test_all_webpages()
{
    ...
    cout << "\nTest 2.3 - Webpages Retrieval (by TLD)" << endl;

    string tlds[] = {"org", "com", "net"};
    string expectedResults[] = {"org20", "com20", "net20"};
    DNS tests[] = {t2, t2, t2, t3, t3, t3};

    for (int i = 0; i < 3; ++i) {
        try {
            tests[i].generateOutput(tests[i].getAllWebpages(tlds[i]));
            output_match(expectedResults[i], 3, i + 1, points);
        } catch (const runtime_error& e) {
            cout << "Error retrieving webpages for TLD " << tlds[i] << ": " << e.what() << endl;
            cout << "Diagnosis: Check if the TLD is valid and if the DNS tree is properly constructed." << endl;
        } catch (const exception& e) {
            cout << "Unexpected error for TLD " << tlds[i] << ": " << e.what() << endl;
            cout << "Diagnosis: Check the implementation of getAllWebpages or generateOutput." << endl;
        }
    }
...
void test_domain_pages()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        int flag = 1; // Reset flag for each domain
        int test_case = i + 1;
        ...
        try {
            ...
            cout << "Error processing domain " << domains[i] << ": " << e.what() << endl;
            cout << "Diagnosis: Check if the domain is valid and the DNS structure is correctly populated." << endl;
            flag = 0; // Mark as failure
        } catch (const exception& e) {
            cout << "Unexpected error for domain " << domains[i] << ": " << e.what() << endl;
            cout << "Diagnosis: Verify the implementation of getDomainPages and related functions." << endl;
            flag = 0; // Mark as failure
        }
