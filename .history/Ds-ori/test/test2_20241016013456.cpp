...
void test_all_webpages()
{
    ...
    cout << "\nTest 2.3 - Webpages Retrival (by TLD)" << endl;

    t2.generateOutput(t2.getAllWebpages("org"));
    output_match("org20", 3, 1, points);
    t2.generateOutput(t2.getAllWebpages("com"));
    output_match("com20", 3, 2, points);
    t2.generateOutput(t2.getAllWebpages("net"));
    output_match("net20", 3, 3, points);
    t3.generateOutput(t3.getAllWebpages("org"));
    output_match("org50", 3, 4, points);
    t3.generateOutput(t3.getAllWebpages("com"));
    output_match("com50", 3, 5, points);
    t3.generateOutput(t3.getAllWebpages("net"));
    output_match("net50", 3, 6, points);
...
void test_domain_pages()
{
    ...
    cout << "\nTest 2.2 - Webpages Retrieval (by domain)" << endl;

    // Load the webpages into the DNS structure
    load_webpages(50, &t2);
    string domains[] = {"fitnessfreaks", "petparadise", "gourmetguide", "greenearth", "cinespot"};
    int num_domains[] = {3, 2, 3, 3, 3};

    f2.open("answers.txt", ios::in);  // Open the answers file
    if (!f2.is_open()) {
        cout << "Error: Could not open answers.txt." << endl;
        return;
    }

    // Loop through each domain to test
    for (int i = 0; i < 5; i++)
    {
        flag = 1;  // Reset flag for each domain
        test_case = i + 1;

        // Generate the output for the domain's webpages
        t2.generateOutput(t2.getDomainPages(domains[i]));

        f1.open("output.txt", ios::in);  // Open the output file
        if (!f1.is_open()) {
            cout << "Error: Could not open output.txt." << endl;
            f2.close();  // Close answers.txt before returning
            return;
        }

        // Read the first line to get the number of pages
        getline(f1, s1);
        int length = stoi(s1);

        // Check if the number of pages matches the expected number
        if (length == num_domains[i])
        {
            // Compare each page's line in the output with the expected answer
            for (int k = 0; k < length; k++)
            {
                getline(f1, s1);  // Read from output.txt
                getline(f2, s2);  // Read from answers.txt
                if (s1 != s2)  // If there's a mismatch
                {
                    flag = 0;  // Mark this test as failed
                    cout << "Mismatch in output for domain " << domains[i] << ": " << s1 << " (expected: " << s2 << ")" << endl;
                    cout << "Diagnosis: Ensure that the correct webpages are being generated for the domain." << endl;
                    break;  // Stop further comparison for this domain
                }
            }
        }
        else  // Mismatch in the number of pages
        {
            flag = 0;  // Mark this test as failed
            cout << "Expected number of pages for domain " << domains[i] << " is " << num_domains[i]
                 << ", but got " << length << endl;
            cout << "Diagnosis: Check the domain lookup logic and ensure the DNS tree contains the expected entries." << endl;
        }

        // If no mismatches were found, increment the points
        if (flag)
        {
            points++;
            print_success();
        ...
        f1.close();  // Close output.txt for this iteration
    }

    f2.close();  // Close answers.txt after all tests
