...
void Human::removeFriend(shared_ptr<Human> Friend) {
    // Ensure both humans are part of the graph (debugging)
    auto thisVertex = this->Friends.getVertex(shared_from_this());
    auto friendVertex = this->Friends.getVertex(Friend);
    
    // Debug: Check if vertices for both humans are found
    if (!thisVertex) {
        cout << "Error: This human is not in the graph!" << endl;
        return; // Return early if the current human is not found
    }
    if (!friendVertex) {
        cout << "Error: Friend is not in the graph!" << endl;
        return; // Return early if the friend is not found
    }

    // Debug: Confirm we are about to remove the edge
    cout << "Removing friendship between " << this->getName() << " and " << Friend->getName() << endl;
    ...
    // Debug: Verify if edges are removed (check adjacency list)
    auto friendsAfterRemoval = this->getFriends();
    cout << this->getName() << "'s friends after removal: ";
