...
vector<shared_ptr<Human>> SocialNetwork::connectionOrder(shared_ptr<Human> human1, shared_ptr<Human> human2) {
    ...
    // Assuming vertices are stored in a vector in the graph
    vector<shared_ptr<Vertex<shared_ptr<Human>>>> vertices = Network.getAllVertices();
    vector<shared_ptr<Vertex<shared_ptr<Human>>>> parent(vertices.size(), nullptr);
    vector<bool> visited(vertices.size(), false);

    queue<shared_ptr<Vertex<shared_ptr<Human>>>> toVisit;
    toVisit.push(vertex1);
    int index1 = -1;
    // Find the index of vertex1
    ...
    while (!toVisit.empty()) {
        ...
        if (currentVertex == vertex2) {
            ...
            while (vertex != nullptr) {
                ...
                // Find the index of the parent vertex
                int parentIndex = -1;
                for (int i = 0; i < vertices.size(); ++i) {
                    if (vertices[i] == parent[Network.getVertexIndex(vertex)]) {
                        parentIndex = i;
                        break;
                    }
                }
