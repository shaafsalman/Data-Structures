...
vector<vector<shared_ptr<Human>>> SocialNetwork::getGroups() {
    // Returns a vector of groups, where each group consists of humans connected to each other
    // either directly or indirectly. 

    vector<vector<shared_ptr<Human>>> groups;  // Stores the groups of humans
    ...
    for (auto& human : Network.getAllVertices()) {
        graph.addVertex(human->getData()->getName());
    }

    // Add edges based on friendships
    for (auto& human : Network.getAllVertices()) {
        for (auto& friendPtr : human->getData()->getFriends()) {
            graph.addEdge(human->getData()->getName(), friendPtr->getName());
        }
    }

    // Get connected components (groups) from the graph
