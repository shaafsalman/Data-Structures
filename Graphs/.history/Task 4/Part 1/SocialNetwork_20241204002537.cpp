...
void Human::removeFriend(shared_ptr<Human> Friend) {
    ...
    if (!thisVertex || !friendVertex) return;  // If any human isn't in the graph, return
    
    // Remove the edge between this human and the friend
    this->Friends.removeEdge(shared_from_this(), Friend);
    this->Friends.removeEdge(Friend, shared_from_this()); // Remove the friendship in both directions
}
