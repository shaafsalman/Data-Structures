...
vector<vector<shared_ptr<Human>>> SocialNetwork::getGroups() {
    ...
    for (auto& human : humans) {
        graph.addVertex(human->getName());
    }

    for (auto& human : humans) {
        for (auto& friendPtr : human->getFriends()) {
            graph.addEdge(human->getName(), friendPtr->getName());
        }
    }
...
bool SocialNetwork::canBeConnected(shared_ptr<Human> human1, shared_ptr<Human> human2) {
    // Solution: Use BFS/DFS to check if there is a path between human1 and human2
    Graph<Human> graph;
    for (auto& human : humans) {
        graph.addVertex(human->getName());
    }
    ...
    auto source = graph.getVertex(human1->getName());
    auto destination = graph.getVertex(human2->getName());

    if (!source || !destination) {
        return false;  // One of the humans doesn't exist in the network
    }

    // Use BFS to check if there is a path between source and destination
    vector<shared_ptr<Vertex<Human>>> path = graph.shortestPath(source, destination);
    return !path.empty();
}

vector<shared_ptr<Human>> SocialNetwork::connectionOrder(shared_ptr<Human> human1, shared_ptr<Human> human2) {
    // Solution: Use BFS/DFS to get the connection order (path) between human1 and human2
    Graph<Human> graph;
    for (auto& human : humans) {
        graph.addVertex(human->getName());
    }
    ...
    if (!source || !destination) {
        return {};  // One of the humans doesn't exist in the network
    }

    vector<shared_ptr<Vertex<Human>>> path = graph.shortestPath(source, destination);
    vector<shared_ptr<Human>> connectionOrder;

    for (auto& vertex : path) {
        connectionOrder.push_back(vertex->getData());
    }
