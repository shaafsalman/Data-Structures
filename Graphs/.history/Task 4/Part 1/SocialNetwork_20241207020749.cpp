...
vector<shared_ptr<Human>> SocialNetwork::connectionOrder(shared_ptr<Human> human1, shared_ptr<Human> human2) {
    ...
    vector<shared_ptr<Vertex<shared_ptr<Human>>>> parent(Network.getVertexCount(), nullptr);
    ...
    while (!toVisit.empty()) {
        ...
        if (currentVertex == vertex2) {
            ...
            while (vertex != nullptr) {
                ...
                vertex = parent[Network.getVertexIndex(vertex)];
            }
        ...
        for (size_t i = 0; i < adjacentVertices.size(); ++i) {
            auto neighbor = adjacentVertices[i];
            int neighborIndex = Network.getVertexIndex(neighbor);
            if (!parent[neighborIndex]) {
                toVisit.push(neighbor);
