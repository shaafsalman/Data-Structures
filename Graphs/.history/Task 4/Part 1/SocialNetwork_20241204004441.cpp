...
void Human::removeFriend(shared_ptr<Human> Friend) {
    ...
    // Ensure the adjacency matrix is updated after removing the edge
    this->Friends.updateAdjacencyMatrix(); // Update matrix to reflect changes in the graph

    // Manually refresh the adjacency list for Alice (thisVertex)
    auto adjList = this->Friends.getAdjacentVertices(thisVertex);

    // Check the list of friends again after removal
    ...
    // Debugging: Ensure that the adjacency list is empty after removal
    cout << "Final adjacency list for " << this->getName() << ": ";
    if (adjList.empty()) {
        cout << "No friends left." << endl; // Alice should have no friends left
    } else {
