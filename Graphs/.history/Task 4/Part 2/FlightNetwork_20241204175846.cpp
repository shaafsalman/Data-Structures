...
Flight::Flight() 
    ...
    std::cout << "Flight constructor: Flight " << flightNumber << " created with status " << status << std::endl;
}
...
Flight::Flight(string flightNumber, shared_ptr<Airport> departureAirport, shared_ptr<Airport> destinationAirport, int distance, int cost, FlightStatus status) 
    ...
    if (destinationAirport) {
        std::cout << "Destination airport: " << destinationAirport->getName() << std::endl;
    }
}
...
FlightStatus Flight::getStatus() {
        std::cout << "getting Status of flight " << flightNumber << "  " << status << std::endl;
    return status;
}

void Flight::setStatus(FlightStatus status) {
    this->status = status;
    std::cout << "Status of flight " << flightNumber << " changed to " << status << std::endl;
}
