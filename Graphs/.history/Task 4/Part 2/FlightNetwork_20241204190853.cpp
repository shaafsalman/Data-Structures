...
Flight::Flight(string flightNumber, shared_ptr<Airport> departureAirport, shared_ptr<Airport> destinationAirport, int distance, int cost, FlightStatus status)
    ...
    // Debugging: Check if departure and destination airports are valid
    if (departureAirport) {
        std::cout << "Adding flight " << flightNumber << " to departure airport " << std::endl;
    } else {
        std::cout << "Error: Departure airport is null for flight " << flightNumber << std::endl;
    }

    if (destinationAirport) {
        std::cout << "Adding flight " << flightNumber << " to destination airport " <<  std::endl;
    } else {
        std::cout << "Error: Destination airport is null for flight " << flightNumber << std::endl;
    }
}
...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    // Ensure shared ownership and add the flight to the departure list
    departureFlights.push_back(flight);
    std::cout << "Added departure flight: " << flight->getFlightNumber() 
              << " from " << getName() << " to " << flight->getDestinationAirport()->getName() << std::endl;

    // Add edges to the graph with shared_ptr<Airport>
    if (departureFlights_distance && departureFlights_cost) {
        shared_ptr<Airport> destination = flight->getDestinationAirport();
        
        // Adding edges to distance and cost graphs
        try {
            departureFlights_distance->addEdge(shared_from_this(), destination, flight->getDistance());
            departureFlights_cost->addEdge(shared_from_this(), destination, flight->getCost());
        } catch (const std::exception& e) {
            std::cout << "Error: Failed to add edges to the departure graphs: " << e.what() << std::endl;
        }
    }
...
void Airport::addArrivalFlight(shared_ptr<Flight> flight) {
    ...
    if (arrivalFlights_distance && arrivalFlights_cost) {
        shared_ptr<Airport> departure = flight->getDepartureAirport();
        
        // Adding edges to distance and cost graphs
        try {
            arrivalFlights_distance->addEdge(departure, shared_from_this(), flight->getDistance());
            arrivalFlights_cost->addEdge(departure, shared_from_this(), flight->getCost());
        } catch (const std::exception& e) {
            std::cout << "Error: Failed to add edges to the arrival graphs: " << e.what() << std::endl;
        }
    }
...
void Airport::TakeOff(const string& flightNumber) {
    // Find the flight in the departure list
    auto flight = findFlightByNumber(flightNumber);
    if (flight) {
        flight->setStatus(FlightStatus::TAKEN_OFF);
        std::cout << "set Status of flight " << flightNumber << " changed to TAKEN_OFF" << std::endl;
    } else {
        std::cout << "Error: Flight " << flightNumber << " not found at " << airportCode << std::endl;
    }
}
