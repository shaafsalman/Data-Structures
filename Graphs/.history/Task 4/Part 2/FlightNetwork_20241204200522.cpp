...
void Airport::addArrivalFlight(string flightNumber, shared_ptr<Airport> source, int cost, int distance, FlightStatus status) {
  ...
  void addDepartureFlight(shared_ptr<Flight> flight) {
        if (!flight) {
            cout << "Error: Flight is null." << endl;
            return;
        }
        departureFlights.push_back(flight);
        try {
            ...
            cout << "Error: Failed to add edges to the departure graphs: " << e.what() << endl;
        }
    }
    ...
    void addArrivalFlight(shared_ptr<Flight> flight) {
        ...
        try {
            ...
            cout << "Error: Failed to add edges to the arrival graphs: " << e.what() << endl;
        }
    }
