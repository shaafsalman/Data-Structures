...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    // Ensure the flight is added to the departure graph properly
       ...
       std::cout << "Adding flight details:" << std::endl;
        ...
        std::cout << "  Status: " << static_cast<int>(flight->getStatus()) << std::endl; // Assuming FlightStatus is an enum

    try {
        departureFlights.push_back(flight);
             
        departureFlights_distance->addEdge(flight->getDepartureAirport()->getName(), flight->getDestinationAirport()->getName(), flight->getDistance());
        departureFlights_cost->addEdge(flight->getDepartureAirport()->getName(), flight->getDestinationAirport()->getName(), flight->getCost());
    } catch (const std::exception& e) {
        std::cerr << "Error: Failed to add edges to the departure graphs: " << e.what() << std::endl;
    }
}
