...
Flight::Flight(string flightNumber, shared_ptr<Airport> departureAirport, shared_ptr<Airport> destinationAirport, int distance, int cost, FlightStatus status)
    : flightNumber(flightNumber),
     ...
     if (departureAirport) {
 shared_ptr<Flight> flightPtr = shared_from_this();  // Ensure this gets a shared_ptr to this Flight object
        departureAirport->addDepartureFlight(flightPtr);}

if (destinationAirport) {
   shared_ptr<Flight> flightPtr = shared_from_this();  // Ensure this gets a shared_ptr to this Flight object
        destinationAirport->addArrivalFlight(flightPtr);}
...
void Airport::TakeOff(string flightNumber) {
    shared_ptr<Flight> flight = getFlight(flightNumber);
    if (flight) {
