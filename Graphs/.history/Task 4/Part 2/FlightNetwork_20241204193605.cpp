...
void Airport::TakeOff(string flightNumber) {
    shared_ptr<Flight> flight = getFlight(flightNumber);  // Ensure this retrieves the correct flight
    if (flight) {
        // Log the current status before taking off
        std::cout << "Setting status of flight " << flightNumber << " to TAKEN_OFF." << std::endl;
        flight->setStatus(FlightStatus::TAKEN_OFF);  // Update the flight's status
...
shared_ptr<Flight> Airport::getFlight(string flightNumber) {
    // Look through all departure flights and find the flight with the matching flightNumber
    for (auto& flight : departureFlights) {
        if (flight->getFlightNumber() == flightNumber) {
            return flight;  // Return the correct flight
        }
    }
    // If the flight isn't found, return nullptr
    return nullptr;
}
