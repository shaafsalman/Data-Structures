...
vector<shared_ptr<Airport>> FlightNetwork::getShortestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    ...
    vector<shared_ptr<Airport>> previousAirport(Airports.size(), nullptr);
    ...
    while (true) {
        ...
        if (currentAirport == destination) {
            ...
            while (currentAirport != nullptr) {
                ...
                // Find the previous airport by its index in the previousAirport vector
                currentAirport = previousAirport[minIndex];
            }
