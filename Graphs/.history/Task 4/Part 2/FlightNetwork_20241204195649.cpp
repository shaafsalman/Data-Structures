...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    // Add to departure graphs
    try {
        // Ensure departureGraphs are initialized
        if (!departureFlights_distance) {
            departureFlights_distance = make_shared<Graph<shared_ptr<Airport>>>();
        }
        if (!departureFlights_cost) {
            departureFlights_cost = make_shared<Graph<shared_ptr<Airport>>>();
        }

        // Ensure shared_from_this() is valid
