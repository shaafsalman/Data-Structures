...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    try {
        departureFlights.push_back(flight);
        // If you're adding it to some graph structure, make sure it's correctly linked
        departureGraph.addEdge(flight->getDepartureAirport(), flight->getDestinationAirport(), flight->getCost());
    } catch (const std::exception& e) {
