...
Flight::Flight(string flightNumber, shared_ptr<Airport> departureAirport, shared_ptr<Airport> destinationAirport, int distance, int cost, FlightStatus status)
    ...
    // Automatically add the flight to the departure airport's departure list
    if (departureAirport) {
        departureAirport->addDepartureFlight(shared_ptr<Flight>(this));
    }

    // Automatically add the flight to the destination airport's arrival list
...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    // Print the list of departure flights for debugging
    std::cout << "Departure Flights for " << getName() << ":" << std::endl;
    for (const auto& f : departureFlights) {
        std::cout << "Flight: " << f->getFlightNumber() << std::endl;
    }
...
void Airport::addArrivalFlight(shared_ptr<Flight> flight) {
    ...
    // Check if source and destination airports exist in the graph
    if (!graph.getVertex(flight->getDepartureAirport()->getName())) {
        std::cout << "Airport " << flight->getDepartureAirport()->getName() 
                  << " not found in the graph. Adding to the graph." << std::endl;
        graph.addVertex(flight->getDepartureAirport()->getName());
    }

    if (!graph.getVertex(flight->getDestinationAirport()->getName())) {
        std::cout << "Airport " << flight->getDestinationAirport()->getName() 
                  << " not found in the graph. Adding to the graph." << std::endl;
        graph.addVertex(flight->getDestinationAirport()->getName());
    }

    // Add the arrival flight to the airport's graph
    graph.addEdge(flight->getDepartureAirport()->getName(), flight->getDestinationAirport()->getName(), flight->getDistance());
    std::cout << "Added arrival flight: " 
