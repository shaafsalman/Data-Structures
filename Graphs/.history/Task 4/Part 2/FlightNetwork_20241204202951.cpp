...
void Airport::TakeOff(string flightNumber) {
    for (auto& flight : departureFlights) {
        if (flight->getFlightNumber() == flightNumber) {
            flight->setStatus(FlightStatus::TAKEN_OFF); // Ensure this modifies the correct flight object
            std::cout << "Flight " << flightNumber << " has taken off from " << getName() << std::endl;
            return;
        }
    }
    std::cout << "Error: Flight " << flightNumber << " not found at " << getName() << std::endl;
}
