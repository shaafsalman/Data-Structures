...
Flight::Flight(string flightNumber, shared_ptr<Airport> departureAirport, shared_ptr<Airport> destinationAirport, int distance, int cost, FlightStatus status)
    ...
    if (departureAirport) {
        departureAirport->addDepartureFlight(shared_ptr<Flight>(this)); // Pass the shared_ptr explicitly
    }
       if (destinationAirport) {
        destinationAirport->addArrivalFlight(shared_ptr<Flight>(this)); // Pass the shared_ptr explicitly
    }
}
...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    departureFlights.push_back(flight);
    std::cout << "Added departure flight: " << flight->getFlightNumber() << " from " << flight->getDepartureAirport()->getName() << " to " << flight->getDestinationAirport()->getName() << std::endl;

    // Add to departure graphs
    try {
        departureFlights_distance->addEdge(shared_ptr<Airport>(this), flight->getDestinationAirport(), flight->getDistance());
        departureFlights_cost->addEdge(shared_ptr<Airport>(this), flight->getDestinationAirport(), flight->getCost());
    } catch (const std::exception& e) {
        std::cout << "Error: Failed to add edges to the departure graphs: " << e.what() << std::endl;
    }
}
