...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    if (departureFlights_distance && !departureFlights_distance->hasVertex(flight->getDepartureAirport())) {
        departureFlights_distance->addVertex(flight->getDepartureAirport());
    }
    if (departureFlights_cost && !departureFlights_cost->hasVertex(flight->getDepartureAirport())) {
...
void Airport::addArrivalFlight(shared_ptr<Flight> flight) {
    ...
    if (arrivalFlights_distance && !arrivalFlights_distance->hasVertex(flight->getDestinationAirport())) {
        arrivalFlights_distance->addVertex(flight->getDestinationAirport());
    }
    if (arrivalFlights_cost && !arrivalFlights_cost->hasVertex(flight->getDestinationAirport())) {
