...
void Airport::addFlight(shared_ptr<Flight> flight) {
    ...
    // Solution:
     if (!hasAirport(flight->getDeparture()->getName()) || 
        !hasAirport(flight->getDestination()->getName())) {
        throw logic_error("One or both airports are not part of the network.");
    }
...
bool Airport::hasFlight(string flightNumber) {
    ...
    // Solution:
       return flights.find(flightNumber) != flights.end();
...
void FlightNetwork::addAirport(shared_ptr<Airport> airport) {
    if (!hasAirport(airport->getName())) {
        ...
        AirportNetwork_distance.addVertex(airport); // Directly pass shared_ptr<Airport>
        AirportNetwork_cost.addVertex(airport);    // Directly pass shared_ptr<Airport>
    }
}
...
void FlightNetwork::addFlight(shared_ptr<Flight> flight) {
    ...
    // Ensure both airports exist in the network
    if (!hasAirport(departureAirport->getName()) || !hasAirport(destinationAirport->getName())) {
        ...
        return; // Flight cannot be added if airports are not in the network
    }

    // Add the flight to the respective airports
    departureAirport->addDepartureFlight(flight);
    destinationAirport->addArrivalFlight(flight);

    // Add the flight to the graph representations
    ...
    std::cout << "Flight " << flight->getFlightNumber() << " added between "
              << departureAirport->getName() << " and " << destinationAirport->getName() << "." << std::endl;
}
