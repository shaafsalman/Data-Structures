...
Flight::Flight(string flightNumber, shared_ptr<Airport> departureAirport, shared_ptr<Airport> destinationAirport, int distance, int cost, FlightStatus status)
    : flightNumber(flightNumber),
      ...
      cost(cost),
      status(status) {
    // Use a weak pointer as an intermediary
    auto weakThis = weak_ptr<Flight>(shared_from_this());
    auto sharedThis = weakThis.lock();
    if (!sharedThis) {
        throw std::runtime_error("Flight object not fully constructed.");
    }

    if (departureAirport) {
        departureAirport->addDepartureFlight(sharedThis);
    }
    if (destinationAirport) {
        destinationAirport->addArrivalFlight(sharedThis);
    }
}
...
void Airport::TakeOff(string flightNumber) {
    // Search for the flight in the departureFlights list
    auto flightIt = std::find_if(departureFlights.begin(), departureFlights.end(),
                                 [&flightNumber](const std::shared_ptr<Flight>& flight) {
                                     return flight && flight->getFlightNumber() == flightNumber;
                                 });

    if (flightIt != departureFlights.end() && *flightIt) {
        // Update the flight status to TAKEN_OFF
        (*flightIt)->setStatus(FlightStatus::TAKEN_OFF);
        std::cout << "Flight " << flightNumber << " has taken off." << std::endl;
    } else {
        // Handle the case where the flight is not found
        std::cout << "Error: Flight " << flightNumber << " not found in departure flights." << std::endl;
    }
}
