...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    try {
        ...
        if (!departureFlights_distance->vertexExists(departureAirport)) {
            departureFlights_distance->addVertex(departureAirport);
        }
        if (!departureFlights_distance->vertexExists(destinationAirport)) {
            departureFlights_distance->addVertex(destinationAirport);
        }

        if (!departureFlights_cost->vertexExists(departureAirport)) {
            departureFlights_cost->addVertex(departureAirport);
        }
        if (!departureFlights_cost->vertexExists(destinationAirport)) {
            departureFlights_cost->addVertex(destinationAirport);
        }

        // Ensure edges don't already exist
        if (!departureFlights_distance->edgeExists(departureAirport, destinationAirport)) {
            departureFlights_distance->addEdge(departureAirport, destinationAirport, flight->getDistance());
        } else {
            std::cout << "Edge already exists in distance graph." << std::endl;
        }

        if (!departureFlights_cost->edgeExists(departureAirport, destinationAirport)) {
            departureFlights_cost->addEdge(departureAirport, destinationAirport, flight->getCost());
        } else {
