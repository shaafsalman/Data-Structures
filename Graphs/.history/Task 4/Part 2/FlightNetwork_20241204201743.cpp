...
void Airport::TakeOff(string flightNumber) {
    ...
    std::cout << "Departure flights from " << getName() << ":" << std::endl;
    for (auto& flight : departureFlights) {
        std::cout << "Flight Number: " << flight->getFlightNumber() << std::endl;
        std::cout << "Destination: " << flight->getDestinationAirport()->getName() << std::endl;
        std::cout << "Status: " << flight->getStatus() << std::endl;  
        std::cout << "Distance: " << flight->getDistance() << " km" << std::endl;
        std::cout << "Cost: " << flight->getCost() << " USD" << std::endl;
        std::cout << "-----------------------------------------" << std::endl;
    }
    
    // Iterate through the flights and set the status of the specified flight to TAKEN_OFF
    for (auto& flight : departureFlights) {
        if (flight->getFlightNumber() == flightNumber) {
            flight->setStatus(FlightStatus::TAKEN_OFF);  // This directly modifies the original shared_ptr<Flight>
