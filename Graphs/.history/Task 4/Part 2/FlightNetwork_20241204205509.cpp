...
void Airport::addDepartureFlight(std::shared_ptr<Flight> flight) {
    auto departureAirport = flight->getSource();
    auto destinationAirport = flight->getDestination();
    int distance = flight->getDistance();
    int cost = flight->getCost();

    // Check if the departure airport exists in the graph
    if (departureFlights_distance->getVertex(departureAirport) == nullptr) {
        departureFlights_distance->addVertex(departureAirport);
    }
    if (departureFlights_distance->getVertex(destinationAirport) == nullptr) {
        departureFlights_distance->addVertex(destinationAirport);
    }

    if (departureFlights_cost->getVertex(departureAirport) == nullptr) {
        departureFlights_cost->addVertex(departureAirport);
    }
    if (departureFlights_cost->getVertex(destinationAirport) == nullptr) {
        departureFlights_cost->addVertex(destinationAirport);
    }

    // Check if the edge exists before adding it
    if (departureFlights_distance->getEdge(departureAirport, destinationAirport) == nullptr) {
        departureFlights_distance->addEdge(departureAirport, destinationAirport, distance);
    }
    if (departureFlights_cost->getEdge(departureAirport, destinationAirport) == nullptr) {
        departureFlights_cost->addEdge(departureAirport, destinationAirport, cost);
    }
}
