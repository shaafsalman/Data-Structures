...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    ...
    // Ensure the departure airport is in the graph before adding a flight
    if (departureFlights_distance && !departureFlights_distance->getVertex(flight->getDepartureAirport())) {
        departureFlights_distance->addVertex(flight->getDepartureAirport());
    }
    if (departureFlights_cost && !departureFlights_cost->getVertex(flight->getDepartureAirport())) {
    ...
    // Add the flight's departure and destination airports to the graph
    try {
...
void Airport::addArrivalFlight(shared_ptr<Flight> flight) {
    ...
    // Ensure the destination airport is in the graph before adding a flight
    if (arrivalFlights_distance && !arrivalFlights_distance->getVertex(flight->getDestinationAirport())) {
        arrivalFlights_distance->addVertex(flight->getDestinationAirport());
    }
    if (arrivalFlights_cost && !arrivalFlights_cost->getVertex(flight->getDestinationAirport())) {
