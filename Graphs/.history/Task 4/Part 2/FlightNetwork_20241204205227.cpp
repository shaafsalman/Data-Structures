...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    // Debug: Print the details of the flight
    std::cout << "Adding flight details:" << std::endl;
    std::cout << "  Departure Airport: " << flight->getDepartureAirport()->getName() << std::endl;
    std::cout << "  Destination Airport: " << flight->getDestinationAirport()->getName() << std::endl;
    std::cout << "  Distance: " << flight->getDistance() << " km" << std::endl;
    std::cout << "  Cost: $" << flight->getCost() << std::endl;
    std::cout << "  Status: " << static_cast<int>(flight->getStatus()) << std::endl;

    try {
        // Get airport names
        string departureName = flight->getDepartureAirport()->getName();
        string destinationName = flight->getDestinationAirport()->getName();

        // Ensure vertices exist before adding
        if (!departureFlights_distance->vertexExists(departureName)) {
            departureFlights_distance->addVertex(departureName);
        }
        if (!departureFlights_distance->vertexExists(destinationName)) {
            departureFlights_distance->addVertex(destinationName);
        }
        if (!departureFlights_cost->vertexExists(departureName)) {
            departureFlights_cost->addVertex(departureName);
        }
        if (!departureFlights_cost->vertexExists(destinationName)) {
            departureFlights_cost->addVertex(destinationName);
        }

        // Ensure edges don't already exist before adding
        if (!departureFlights_distance->edgeExists(departureName, destinationName)) {
            departureFlights_distance->addEdge(departureName, destinationName, flight->getDistance());
        } else {
            std::cout << "Edge already exists in distance graph." << std::endl;
        }
        if (!departureFlights_cost->edgeExists(departureName, destinationName)) {
            departureFlights_cost->addEdge(departureName, destinationName, flight->getCost());
        } else {
            std::cout << "Edge already exists in cost graph." << std::endl;
        }

        // Add the flight to the departure list
        departureFlights.push_back(flight);

        std::cout << "Flight successfully added to the graphs." << std::endl;
    } catch (const std::exception &e) {
        std::cerr << "Error while adding flight to the graphs: " << e.what() << std::endl;
    }
}
...
void Airport::addArrivalFlight(shared_ptr<Flight> flight) {
    ...
    arrivalFlights.push_back(flight);
    std::cout << "Added arrival flight: " << flight->getFlightNumber() << " from " << flight->getDepartureAirport()->getName() << " to " << getName() << std::endl;
}
...
void Airport::TakeOff(string flightNumber) {
    if (departureFlights.empty()) {
        std::cout << "No departure flights from " << getName() << std::endl;
        return;
    }

    for (auto& flight : departureFlights) {
        std::cout << "Flight in departure list: " << flight->getFlightNumber() << " Status: " << flight->getStatus() << std::endl;
    ...
    std::cout << "Flight " << flightNumber << " not found at " << getName() << std::endl;
}
