...
void Airport::addDepartureFlight(shared_ptr<Flight> flight) {
    std::cout << "Adding Departure Flight: -----------------------." << std::endl;
    ...
    std::cout << "Airport Name: " << getName() << std::endl;
    std::cout << "Airport City: " << getCity() << std::endl;
    std::cout << "Airport Country: " << getCountry() << std::endl;
    ...
    // Add to departure graphs
    try {
        // Ensure shared pointer ownership
        if (auto sharedThis = shared_from_this()) {
            departureFlights_distance->addEdge(sharedThis, flight->getDestinationAirport(), flight->getDistance());
            departureFlights_cost->addEdge(sharedThis, flight->getDestinationAirport(), flight->getCost());
        } else {
            std::cout << "Error: Failed to create shared pointer for Airport object." << std::endl;
        }
    } catch (const std::exception& e) {
        std::cout << "Error: Failed to add edges to the departure graphs: " << e.what() << std::endl;
    }
...
void Airport::addArrivalFlight(shared_ptr<Flight> flight) {
    ...
    try {
        // Ensure shared pointer ownership
        if (auto sharedThis = shared_from_this()) {
            arrivalFlights_distance->addEdge(flight->getDepartureAirport(), sharedThis, flight->getDistance());
            arrivalFlights_cost->addEdge(flight->getDepartureAirport(), sharedThis, flight->getCost());
        } else {
            std::cout << "Error: Failed to create shared pointer for Airport object." << std::endl;
        }
    } catch (const std::exception& e) {
