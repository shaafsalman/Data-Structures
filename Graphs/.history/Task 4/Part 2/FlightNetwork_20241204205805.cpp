...
void Airport::addDepartureFlight(const string& flightNumber, shared_ptr<Airport> destination, int cost, int distance, FlightStatus status) {
    if (!destination || flightNumber.empty()) {
        std::cout << "Error: Invalid parameters. Destination airport or flight number is null/empty." << std::endl;
        return;
    }
    ...
    for (auto [graph, value] : {{departureFlights_distance, distance}, {departureFlights_cost, cost}}) {
        try {
            if (!graph->getVertex(shared_from_this())) graph->addVertex(shared_from_this());
            if (!graph->getVertex(destination)) graph->addVertex(destination);
            graph->addEdge(shared_from_this(), destination, value);
        } catch (const std::exception& e) {
            std::cout << "Error: Failed to update graph: " << e.what() << std::endl;
        }
    }
}

void Airport::addArrivalFlight(const string& flightNumber, shared_ptr<Airport> source, int cost, int distance, FlightStatus status) {
    if (!source || flightNumber.empty()) {
        std::cout << "Error: Invalid parameters. Source airport or flight number is null/empty." << std::endl;
        return;
    }
    ...
    for (auto [graph, value] : {{arrivalFlights_distance, distance}, {arrivalFlights_cost, cost}}) {
        try {
            if (!graph->getVertex(source)) graph->addVertex(source);
            if (!graph->getVertex(shared_from_this())) graph->addVertex(shared_from_this());
            graph->addEdge(source, shared_from_this(), value);
        } catch (const std::exception& e) {
            std::cout << "Error: Failed to update graph: " << e.what() << std::endl;
        }
    }
...
void Airport::addDepartureFlight(std::shared_ptr<Flight> flight) {
    ...
    auto destination = flight->getDepartureAirport();
    int distance = flight->getDistance(), cost = flight->getCost();

    for (auto graph : {departureFlights_distance, departureFlights_cost}) {
        if (!graph->getVertex(departure)) graph->addVertex(departure);
        if (!graph->getVertex(destination)) graph->addVertex(destination);
    }

    if (!departureFlights_distance->getEdge(departure, destination))
        departureFlights_distance->addEdge(departure, destination, distance);
    if (!departureFlights_cost->getEdge(departure, destination))
        departureFlights_cost->addEdge(departure, destination, cost);
}
...
void Airport::addArrivalFlight(std::shared_ptr<Flight> flight) {
    if (flight) arrivalFlights.push_back(flight);
}
