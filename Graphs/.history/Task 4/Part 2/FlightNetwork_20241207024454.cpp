...
vector<shared_ptr<Airport>> FlightNetwork::getShortestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    ...
    // Initialize the distance for the source airport
    ...
    while (true) {
        ...
        // If no such airport is found, or we've reached the destination, stop
        if (currentIndex == -1 || airports[currentIndex] == destination) {
            break;
        }

        visited[currentIndex] = true;
        shared_ptr<Airport> currentAirport = airports[currentIndex];

        // Update distances for neighboring airports
        for (const auto& flight : currentAirport->getAllFlights()) {
            shared_ptr<Airport> neighbor = flight->getDestinationAirport();

            // Find the index of the neighboring airport
            int neighborIndex = -1;
            for (int i = 0; i < airports.size(); ++i) {
                if (airports[i] == neighbor) {
                    neighborIndex = i;
                    break;
                }
            }
    ...
    while (currentIndex != -1) {
        path.push_back(airports[currentIndex]);
...
vector<shared_ptr<Airport>> FlightNetwork::getCheapestPath(shared_ptr<Airport> source, shared_ptr<Airport> destination) {
    ...
    auto getIndex = [&](shared_ptr<Airport> airport) {
        return std::distance(airports.begin(), std::find(airports.begin(), airports.end(), airport));
    };
    ...
    for (size_t i = 0; i < airports.size(); ++i) {
        ...
        if (currentIndex == -1 || costs[currentIndex] == std::numeric_limits<double>::infinity()) {
            break;
        }
