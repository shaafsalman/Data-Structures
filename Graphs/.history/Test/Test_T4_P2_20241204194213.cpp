...
int main() {
    ...
    try {
        shared_ptr<Flight> flight1 = make_shared<Flight>("AA100", airport1, airport2, 4000, 300, FlightStatus::SCHEDULED);
// Output the status before takeoff
std::cout << "Before TakeOff: " << flight1->getStatus() << std::endl;

// Take off the flight
airport1->TakeOff("AA100");

// Output the status after takeoff
std::cout << "After TakeOff: " << flight1->getStatus() << std::endl;
