...
int main() {
    ...
    // Test 4: Strongly Connected Components (5 marks)
    cout << "Test 4: Strongly Connected Components (5 marks)" << endl;
    try {
        Graph<int> graph3(true, true); // Directed and Weighted graph
        graph3.addVertex(1);
        graph3.addVertex(2);
        ...
        auto scc = graph3.stronglyConnectedComponents();
        cout << "Strongly Connected Components: " << endl;
        int sccCount = 0;
        for (const auto& component : scc) {
            cout << "Component " << ++sccCount << ": ";
            for (const auto& vertex : component) {
                cout << vertex->getData() << " ";
            }
            cout << endl;
        }
        if (scc.size() == 2) { // Assuming we have two SCCs
            cout << "Strongly Connected Components passed! +5 marks" << endl;
            totalScore += 5;
        } else {
            cout << "Strongly Connected Components failed." << endl;
        }
        cout << endl;
    } catch (const exception& e) {
        cout << "Strongly Connected Components failed." << endl;
