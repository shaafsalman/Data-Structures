...
vector<shared_ptr<Graph<T>>> Graph<T>::SpanningTrees() {
    ...
    vector<shared_ptr<Edge<T>>> allEdges = getAllEdges();

    // Generate all combinations of edges of size V - 1
    int V = vertices.size();  // Number of vertices in the graph
    if (V == 0) {
        return spanningTrees; // No spanning trees possible in an empty graph
    }

    int edgeCount = allEdges.size();
    vector<bool> included(edgeCount, false);

    // Helper function to check if a given set of edges forms a valid spanning tree
    auto isSpanningTree = [this](const vector<shared_ptr<Edge<T>>>& edges) -> bool {
        if (edges.size() != vertices.size() - 1) {
            return false; // A tree must have exactly V - 1 edges
        }

        // Use Union-Find to detect cycles and check connectivity
        unordered_map<shared_ptr<Vertex<T>>, shared_ptr<Vertex<T>>> parent;
        unordered_map<shared_ptr<Vertex<T>>, int> rank;

        auto findParent = [&parent](shared_ptr<Vertex<T>> vertex) -> shared_ptr<Vertex<T>> {
        ...
        auto unionSets = [&parent, &rank, &findParent](shared_ptr<Vertex<T>> u, shared_ptr<Vertex<T>> v) {
        ...
        // Try to union the vertices using the given edges
        for (const auto& edge : edges) {
            if (!unionSets(edge->getSource(), edge->getDestination())) {
                return false; // Cycle detected, not a valid spanning tree
            }
        }
        return true;
    };

    // Generate combinations of V - 1 edges and check for valid spanning trees
    // Helper function to recursively generate combinations of edges of size V - 1
    function<void(int, int)> generateCombinations = [&](int idx, int count) {
        if (count == V - 1) {
            // We have selected V - 1 edges, check if they form a valid spanning tree
            vector<shared_ptr<Edge<T>>> selectedEdges;
            for (int i = 0; i < edgeCount; ++i) {
                if (included[i]) {
                    selectedEdges.push_back(allEdges[i]);
                }
            }
            if (isSpanningTree(selectedEdges)) {
                // If it's a valid spanning tree, create a new graph and add it to the result
                shared_ptr<Graph<T>> mst = make_shared<Graph<T>>(false, weighted);
...
vector<vector<shared_ptr<Vertex<T>>>> Graph<T>::connectedComponents() {
    ...
    function<void(shared_ptr<Vertex<T>>, vector<shared_ptr<Vertex<T>>>&)> dfs = [&](shared_ptr<Vertex<T>> vertex, vector<shared_ptr<Vertex<T>>>& component) {
        visited[vertex] = true;
        component.push_back(vertex);
        for (auto adj : getAdjacentVertices(vertex)) {
            if (!visited[adj]) {
