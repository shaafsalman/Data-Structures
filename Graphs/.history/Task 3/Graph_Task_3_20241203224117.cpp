...
vector<shared_ptr<Vertex<T>>> Graph<T>::shortestPath(shared_ptr<Vertex<T>> source, shared_ptr<Vertex<T>> destination) {
    if (weighted) {
        ...
        auto comparator = [&](shared_ptr<Vertex<T>> a, shared_ptr<Vertex<T>> b) {
            return distances[a] > distances[b];
        };
        priority_queue<shared_ptr<Vertex<T>>, vector<shared_ptr<Vertex<T>>>, decltype(comparator)> pq(comparator);
        ...
        // Use BFS for unweighted graphs (existing implementation)
