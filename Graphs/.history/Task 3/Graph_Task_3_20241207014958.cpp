...
vector<vector<shared_ptr<Vertex<T>>>> Graph<T>::stronglyConnectedComponents() {
    ...
    // First DFS to fill the finish order (using DFS traversal on the original graph)
    ...
    while (!finishOrder.empty()) {
        shared_ptr<Vertex<T>> vertex = finishOrder.back();
        finishOrder.pop_back();

        if (!visitedReversed[vertex->getIndex()]) {
