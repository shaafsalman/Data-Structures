...
vector<shared_ptr<Graph<T>>> Graph<T>::SpanningTrees() {
    ...
    // Helper DFS function to explore a graph and build a spanning tree
    auto dfsSpanningTree = [&spanningTrees](shared_ptr<Vertex<T>> vertex, auto&& dfsSpanningTree, 
                                             unordered_map<shared_ptr<Vertex<T>>, bool>& visited, 
    ...
    // Perform DFS to create a spanning tree from every unvisited vertex
    unordered_map<shared_ptr<Vertex<T>>, bool> visited;
    for (auto vertex : vertices) {
        if (!visited[vertex]) {
            // Create a new graph object to store the spanning tree
