...
vector<vector<shared_ptr<Vertex<T>>>> Graph<T>::connectedComponents() {
    ...
    std::function<void(shared_ptr<Vertex<T>>)> dfs = [&](shared_ptr<Vertex<T>> vertex) {
        visited[vertex] = true;
    ...
    for (auto& vertex : vertices) {
        if (!visited[vertex]) {
            ...
            // Run DFS to find all vertices in the connected component
            dfs(vertex);

            // Collect all the visited vertices as part of this component
            for (auto& v : visited) {
                if (v.second) {  // If the vertex is visited
                    component.push_back(v.first);
                    visited[v.first] = false;  // Reset visited for the next component
                }
            }
