...
auto dfsOriginal = [&visited, &finishOrder](shared_ptr<Vertex<T>> vertex, auto&& dfsOriginal, Graph<T>& graph) -> void {
    visited[vertex] = true;
    for (auto neighbor : graph.getAdjacentVertices(vertex)) {  // Call getAdjacentVertices through the graph object
        if (!visited[neighbor]) {
            dfsOriginal(neighbor, dfsOriginal, graph);  // Pass all three arguments
        }
    }
    finishOrder.push(vertex);
};

// When you call the lambda, pass *this (Graph object) explicitly:
dfsOriginal(vertex, dfsOriginal, *this);
