...
vector<vector<shared_ptr<Vertex<T>>>> Graph<T>::connectedComponents() {
    ...
    // Use std::function to define the DFS lambda
    std::function<void(shared_ptr<Vertex<T>>, vector<shared_ptr<Vertex<T>>>&)> dfs = [&](shared_ptr<Vertex<T>> vertex, vector<shared_ptr<Vertex<T>>>& component) {
        visited[vertex] = true;
