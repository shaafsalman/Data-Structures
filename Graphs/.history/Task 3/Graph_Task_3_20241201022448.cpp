...
shared_ptr<Graph<T>> Graph<T>::minimumSpanningTree() {
    // Ensure that the graph is undirected, as MST is only defined for undirected graphs
    if (directed) {
    ...
    shared_ptr<Graph<T>> mst = make_shared<Graph<T>>(false, true);  // Undirected graph with weighted edges
    ...
    sort(edges.begin(), edges.end(), [](const shared_ptr<Edge<T>>& a, const shared_ptr<Edge<T>>& b) {
        return a->getWeight() < b->getWeight();  // Compare based on edge weight
    });
    ...
    for (auto& edge : edges) {
        ...
        // If u and v belong to different sets, add the edge to the MST and union the sets
