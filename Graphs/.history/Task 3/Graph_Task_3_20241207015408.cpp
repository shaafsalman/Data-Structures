...
vector<shared_ptr<Graph<T>>> Graph<T>::SpanningTrees() {
    ...
    dfsSpanningTree = [&](shared_ptr<Vertex<T>> vertex, vector<bool>& visited, shared_ptr<Graph<T>> tree) {
        int idx = -1;
        for (int i = 0; i < vertices.size(); ++i) {
        ...
        for (auto neighbor : this->getAdjacentVertices(vertex)) { 
            ...
            if (!visited[neighborIdx]) {
                tree->addEdge(vertex->getData(), neighbor->getData()); 
                dfsSpanningTree(neighbor, visited, tree); 
            }
        }
    };
    ...
    for (auto vertex : vertices) {
        ...
        if (!visited[idx]) {
            ...
            dfsSpanningTree(vertex, visited, tree);  
