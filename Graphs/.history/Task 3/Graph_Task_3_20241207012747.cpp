...
auto dfsOriginal = [this, &visited, &finishOrder](shared_ptr<Vertex<T>> vertex, auto&& dfsOriginal) -> void {
    visited[vertex] = true;
    for (auto neighbor : this->getAdjacentVertices(vertex)) {  // Capture 'this' to call member function
        if (!visited[neighbor]) {
