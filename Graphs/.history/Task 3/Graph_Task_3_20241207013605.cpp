...
template <class T>
void Graph<T>::DFSRevUtil(shared_ptr<Vertex<T>> vertex, vector<shared_ptr<Vertex<T>>>& visited, vector<shared_ptr<Vertex<T>>>& scc) {
    visited.push_back(vertex);
    vertex->setVisited(true);
    ...
    for (auto& edge : edges) {
        shared_ptr<Vertex<T>> adjacent = edge->getDestination();
        if (!adjacent->isVisited()) {
            DFSRevUtil(adjacent, visited, scc);
        }
    }
}


// Function to find the SCCs in the graph using Kosaraju's Algorithm
template <class T>
vector<vector<shared_ptr<Vertex<T>>>> Graph<T>::kosarajuSCC() {
    vector<vector<shared_ptr<Vertex<T>>>> sccList;
    stack<shared_ptr<Vertex<T>>> stack;

    // Step 1: DFS on the original graph to get the finishing order of vertices
    ...
    while (!stack.empty()) {
        shared_ptr<Vertex<T>> vertex = stack.top();
        stack.pop();

        // If the vertex is not visited, it means it's part of a new SCC
        if (!vertex->isVisited()) {
            vector<shared_ptr<Vertex<T>>> scc;
            DFSRevUtil(vertex, visited, scc);
            sccList.push_back(scc);  // Add the current SCC
        }
    }
...
// Function to reverse the graph (i.e., reverse all edges)
template <class T>
Graph<T> Graph<T>::reverseGraph() {
    ...
    // Reverse all edges
    for (auto& edge : edges) {
        shared_ptr<Vertex<T>> source = edge->getSource();
        shared_ptr<Vertex<T>> destination = edge->getDestination();
        reversedGraph.addEdge(destination, source, edge->getWeight());
    }
