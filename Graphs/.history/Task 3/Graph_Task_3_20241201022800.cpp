...
shared_ptr<Graph<T>> Graph<T>::minimumSpanningTree() {
    // Ensure the graph is undirected, as MST is only defined for undirected graphs
    if (directed) {
    ...
    shared_ptr<Graph<T>> mst = make_shared<Graph<T>>(false, weighted); // Undirected graph with weighted edges
    ...
    // Helper function to find the representative (parent) of a vertex
    auto findParent = [&parent](shared_ptr<Vertex<T>> vertex) -> shared_ptr<Vertex<T>> {
        // If the vertex does not have a parent, it is its own root
        if (parent.find(vertex) == parent.end()) {
            parent[vertex] = vertex; // Initially, each vertex is its own parent
        }

        // Path compression: make the parent of each vertex point directly to the root
        if (parent[vertex] != vertex) {
            parent[vertex] = findParent(parent[vertex]); // Recursively find the root
        }

        return parent[vertex];
    };

    // Helper function to unite two sets (vertices) in the Union-Find structure
    auto unionSets = [&parent, &rank, &findParent](shared_ptr<Vertex<T>> u, shared_ptr<Vertex<T>> v) {
        shared_ptr<Vertex<T>> rootU = findParent(u); // Find root of u
        shared_ptr<Vertex<T>> rootV = findParent(v); // Find root of v
        ...
        if (rootU != rootV) {
            ...
            if (rank[rootU] > rank[rootV]) {
                ...
                parent[rootV] = rootU; // If equal rank, make rootU the parent of rootV
                rank[rootU]++; // Increment the rank of rootU
            }
        }
    };
    ...
    for (auto& edge : edges) {
        ...
        // If u and v belong to different sets, add the edge to the MST and unite the sets
