...
vector<vector<shared_ptr<Vertex<T>>>> Graph<T>::stronglyConnectedComponents() {
    ...
    // Create a visited array for original graph DFS
    unordered_set<shared_ptr<Vertex<T>>> visited;  // Using unordered_set for visited check
    
    // First DFS to fill the finish order
    stack<shared_ptr<Vertex<T>>> finishOrder;
    for (auto& vertex : vertices) {
        if (visited.find(vertex) == visited.end()) {  // If not visited
            vector<shared_ptr<Vertex<T>>> traversal = DFSTraversal(vertex);
    ...
    // Create a visited array for the reversed graph DFS
    unordered_set<shared_ptr<Vertex<T>>> visitedReversed;
    while (!finishOrder.empty()) {
        shared_ptr<Vertex<T>> vertex = finishOrder.top();
        finishOrder.pop();

        if (visitedReversed.find(vertex) == visitedReversed.end()) {  // If not visited
            // Perform DFS on reversed graph to discover strongly connected components
