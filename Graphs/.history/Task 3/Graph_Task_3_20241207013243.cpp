...
vector<vector<shared_ptr<Vertex<T>>>> Graph<T>::stronglyConnectedComponents() {
    ...
    for (auto vertex : vertices) {  // Make sure vertices is defined
        if (!visited[vertex]) {
            ...
            dfsOriginal(vertex, dfsOriginal, *this);  // Pass all 3 arguments: vertex, dfsOriginal, *this
        }
    }

    // Now proceed with the second DFS
    ...
    while (!finishOrder.empty()) {
        ...
        if (!visitedReversed[vertex]) {
            ...
            dfsReversed(vertex, dfsReversed, *this);  // Pass all 3 arguments here as well
