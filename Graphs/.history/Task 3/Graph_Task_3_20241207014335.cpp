...
shared_ptr<Graph<T>> Graph<T>::minimumSpanningTree() {
    ...
    // Create a list of edges, each consisting of a weight, source, and destination vertex
    vector<shared_ptr<Edge<T>>> edgeList = getAllEdges();
    vector<pair<int, pair<shared_ptr<Vertex<T>>, shared_ptr<Vertex<T>>>>> edges; // (weight, (source, destination))
    ...
    // Union-Find: Initialize parent array to represent disjoint sets
    int n = vertices.size();
    vector<int> parent(n), rank(n, 0);

    // Initialize parent of each vertex to itself
    for (int i = 0; i < n; ++i) {
        parent[i] = i;
    }
    ...
    // Process edges in sorted order to construct MST
    for (auto &[weight, edge] : edges) {
        ...
        // If u and v are in different sets, add the edge to MST and union the sets
