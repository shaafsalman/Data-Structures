...
#include <queue>
#include <stack>
#include <unordered_map>  // Add this include to fix the unordered_map errors
...
vector<shared_ptr<Vertex<T>>> Graph<T>::dijkstraShortestPath(shared_ptr<Vertex<T>> source) {
    ...
    // Define a priority queue to pick the vertex with the smallest distance
    ...
    while (!pq.empty()) {
        ...
        // If the distance is greater than the current known distance, skip it
